...
void SKNanoLoader::SetMaxLeafSize(){
    ...
    Muon_ptErr.resize(kMaxMuon);
    Muon_tunepRelPt.resize(kMaxMuon);
    Muon_segmentComp.resize(kMaxMuon);
    Muon_nStations.resize(kMaxMuon);
    Muon_miniPFRelIso_chg.resize(kMaxMuon);
    Muon_pfRelIso03_chg.resize(kMaxMuon);
    Muon_jetPtRelv2.resize(kMaxMuon);
    Muon_jetRelIso.resize(kMaxMuon);
    Muon_jetNDauCharged.resize(kMaxMuon);
    Muon_isPFcand.resize(kMaxMuon);
    Muon_highPurity.resize(kMaxMuon);
    Muon_tightCharge.resize(kMaxMuon);
    if(Run == 3){
        Muon_mvaMuID_WP.resize(kMaxMuon);
        Muon_genPartIdx.resize(kMaxMuon);
        Muon_jetIdx.resize(kMaxMuon);
        Muon_svIdx.resize(kMaxMuon);
        ...
        Muon_svIdx.resize(0);   // Muon_svIdx does not exist in NanoAODv9
    ...
    if(Run == 3){
        ...
        // SV 는 NanoAOD v9 에도 존재한다 (nSV 만 UInt_t) → Run2 에서도 읽는다
        SV_pt.resize(kMaxSV);
        SV_eta.resize(kMaxSV);
        SV_phi.resize(kMaxSV);
        SV_mass.resize(kMaxSV);
...
void SKNanoLoader::InitTTreeReader() {
    ...
    BindArray("Muon_ptErr", Muon_ptErr, &nMuon, "nMuon");
    BindArray("Muon_tunepRelPt", Muon_tunepRelPt, &nMuon, "nMuon");
    BindArray("Muon_segmentComp", Muon_segmentComp, &nMuon, "nMuon");
    BindArray("Muon_nStations", Muon_nStations, &nMuon, "nMuon");
    BindArray("Muon_miniPFRelIso_chg", Muon_miniPFRelIso_chg, &nMuon, "nMuon");
    BindArray("Muon_pfRelIso03_chg", Muon_pfRelIso03_chg, &nMuon, "nMuon");
    BindArray("Muon_jetPtRelv2", Muon_jetPtRelv2, &nMuon, "nMuon");
    BindArray("Muon_jetRelIso", Muon_jetRelIso, &nMuon, "nMuon");
    BindArray("Muon_jetNDauCharged", Muon_jetNDauCharged, &nMuon, "nMuon");
    BindArray("Muon_isPFcand", Muon_isPFcand, &nMuon, "nMuon");
    BindArray("Muon_highPurity", Muon_highPurity, &nMuon, "nMuon");
    BindArray("Muon_tightCharge", Muon_tightCharge, &nMuon, "nMuon");
    ...
    if (Run == 3) {
        ...
        BindArray("Muon_svIdx", Muon_svIdx, &nMuon, "nMuon");
    } else if(Run == 2) {
    ...
    // SV (IVF secondary vertices)----------------------------
    // Run2/Run3 공통: 브랜치는 v9 / v12 양쪽에 모두 있고 SV_* 의 leaf type 도 같다.
    // 다른 것은 counter 뿐이라 (v9 = UInt_t, v12 = Int_t) BindScalarWithRunCheck 가 흡수한다.
...
void SKNanoLoader::InitLegacy() {
    ...
    // NOTE: the extra muon branches added for the flavor-BDT ntuple (ptErr, tunepRelPt,
    // segmentComp, nStations, miniPFRelIso_chg, pfRelIso03_chg, jetPtRelv2, jetRelIso,
    // jetNDauCharged, isPFcand, highPurity, tightCharge, svIdx) are deliberately NOT bound
    // here. This legacy path is only used by Skim* analyzers (SkimmingMode), which clone the
    // raw NanoAOD and never read these members; and SetBranchAddress cannot convert types,
    // so Muon_nStations / Muon_tightCharge (Int_t in v9, UChar_t in v12+) would be misread.
    // In the legacy path they stay zero-filled, exactly as before.
    if (Run == 3) {
    ...
    // SV (IVF secondary vertices)----------------------------
    // InitTTreeReader 쪽과 같은 이유로 Run2/Run3 공통 (counter type 만 다르다)
