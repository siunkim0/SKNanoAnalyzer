...
//   (플래그 없음)  - 세 경로 모두 측정
//   MakeTree      - neural fake factor 학습용 flat ntuple (tree "el")
...
public:
    ...
    bool MakeTree;   // neural fake factor 학습용 flat ntuple
    ...
    //   parton/hadron flavour -> {b,c,s,d,u,g,pileup,unmatched} 문자열 (fake.cc 와 동일)
    ...
    //   reco: electron 의 NanoAOD jetIdx 가 가리키는 jet, gen: 최근접 gen jet (dR<0.4)
    ...
    //   SetBranch 이 string 을 못 쓰므로 tree 에는 정수 코드로 넣는다 (fake.cc 와 동일)
