...
public:
    ...
    RVec<Float_t> Muon_ptErr;
    RVec<Float_t> Muon_tunepRelPt;
    RVec<Float_t> Muon_segmentComp;
    RVec<Int_t> Muon_nStations;   // Int_t in NanoAODv9, UChar_t in v12+ (TTreeReader path casts)
    RVec<Float_t> Muon_miniPFRelIso_chg;
    RVec<Float_t> Muon_pfRelIso03_chg;
    RVec<Float_t> Muon_jetPtRelv2;
    RVec<Float_t> Muon_jetRelIso;
    RVec<UChar_t> Muon_jetNDauCharged;
    RVec<Bool_t> Muon_isPFcand;
    RVec<Bool_t> Muon_highPurity;
    RVec<Int_t> Muon_tightCharge;   // Int_t in NanoAODv9, UChar_t in v12+
    // Run3
    RVec<UChar_t> Muon_mvaMuID_WP;
    RVec<Short_t> Muon_genPartIdx;
    RVec<Short_t> Muon_jetIdx;
    RVec<Short_t> Muon_svIdx;   // NanoAODv12+ only (absent in Run2 v9)
    // Run2
    ...
    // SV (secondary vertices from IVF; Run2 v9 / Run3 v12 양쪽 모두 존재)
    //   counter 만 leaf type 이 다르다: v9 = UInt_t (nSV_RunII), v12 = Int_t (nSV)
    Int_t nSV = 0;
    UInt_t nSV_RunII = 0;
    RVec<Float_t> SV_pt;
    RVec<Float_t> SV_eta;
